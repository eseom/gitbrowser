...
from flamengo.admin import admin as admin_app
from flamengo.models import db

OPTIONS = docopt(__doc__) if __name__ == '__main__' else dict()


class CustomFormatter(logging.Formatter):
    LEVEL_MAP = {logging.FATAL: 'F', logging.ERROR: 'E', logging.WARN: 'W',
...
def setup_logging(name=None):
    ...
    if log_to_disk:
        file_name = os.path.join(OPTIONS['--log_dir'],
                                 'pypi_portal_{}.log'.format(name))
...
def command(func):
    ...
    if func.__name__ not in OPTIONS:
        raise KeyError(
            'Cannot register {}, not mentioned in docstring/docopt.'.format(
                func.__name__))
...
def celerydev():
    ...
    app = create_app(parse_options())
    # Logging._setup = True  # Disable Celery from setting up logging, already
    #  done in setup_logging().
...
def celerybeat():
    ...
    app = create_app(parse_options())
    Logging._setup = True
    celery_args = ['celery', 'beat', '-C', '--pidfile', OPTIONS['--pid'], '-s',
                   OPTIONS['--schedule']]
    with app.app_context():
        return celery_main(celery_args)
...
def celeryworker():
    ...
    app = create_app(parse_options())
    Logging._setup = True
    celery_args = ['celery', 'worker', '-n', OPTIONS['--name'], '-C',
...
@command
def admin():
    dbnum = os.environ.get('DB')
    if not dbnum:
        raise Exception('no DB env')
...
def sink_manager():
    ...
    def app_get_uri(dummy):
        ...
        if not dbnum:
            raise Exception('no DB env')
        return app.config['SQLALCHEMY_URI_TMPL'] % dbnum

    app.get_uri = app_get_uri
...
if __name__ == '__main__':
    signal.signal(signal.SIGINT,
                  lambda *_: sys.exit(0))  # Properly handle Control+C
    if not OPTIONS['--port'].isdigit():
        print('ERROR: Port should be a number.')
        sys.exit(1)
    try:
        getattr(command,
                'chosen')()  # Execute the function specified by the user.
    except AttributeError:
        manager = sink_manager()
        manager.run()
