"""
@refer: https://github.com/walkerh/wsgi-git-http-backend/blob/master
/gitHttpBackend.py
"""
...
def run_git_http_backend(cgi_environ, input_stream, error_stream):
    ...
    proc = subprocess.Popen(
        [current_app.config['GIT_EXECUTABLE'], 'http-backend'],
...
def build_cgi_environ(wsgi_environ, git_project_root, user=None):
    ...
    cgi_environ = dict(
        [(k, v,) for k, v in cgi_environ.items() if isinstance(v, str)])
...
def _communicate_with_git(proc, input_stream, input_length):
    ...
    _input_data_pump(proc, input_stream, input_length)
...
def _find_header_end_in_2_chunks(chunk0, chunk1):
    ...
    chunk0 = bytearray(chunk0, 'utf-8')
    chunk1 = bytearray(chunk1)
...
def _search_str_for_header_end(data_bytes):
    """Return index of header end or -1."""
    return data_bytes.find(HEADER_END)
...
def _separate_header(chunks, header_end_on_boundary, index_within_chunk):
    ...
    header_chunks.append(last_header_chunk[:index_within_chunk].decode('utf-8'))
    header_chunks.append(CRLF.decode('utf-8'))  # Line end might have been
    # split.
